...
// PrivateAdminAPI是仅通过安全RPC通道公开的管理API方法的集合。
...
// AddPeer请求连接到远程节点，并维护新的在任何时候连接，甚至在丢失时重新连接。
...
// PeerEvents创建一个RPC订阅，它从节点的p2p.Server接收对等事件
...
// PublicAdminAPI是通过安全和不安全RPC通道公开的管理API方法的集合。
// PublicAdminAPI is the collection of administrative API methods exposed over
// both secure and unsecure RPC channels.
type PublicAdminAPI struct {
	node *Node // Node interfaced by this API由此API接口的节点
}

//// NewPublicAdminAPI为节点本身的公共管理方法创建一个新的API定义。
...
// Peers以协议粒度检索我们所知道的关于每个单独对等体的所有信息。
...
// NodeInfo以协议粒度检索我们所知道的关于主机节点的所有信息。
