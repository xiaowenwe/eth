...
// Strict is the DB 'strict level'.严格是DB的严格级别
type Strict uint

const (
	// If present then a corrupted or invalid chunk or block in manifest
	// journal will cause an error instead of being dropped.如果存在，则在清单中出现损坏或无效的块或块。	日记将导致错误，而不是被删除。这将防止打开已损坏清单的数据库。
	// This will prevent database with corrupted manifest to be opened.
	StrictManifest Strict = 1 << iota //精确的清单

	// If present then journal chunk checksum will be verified.如果存在，则将对日志块校验和进行验证。
	StrictJournalChecksum //严格日志校验和

	// If present then a corrupted or invalid chunk or block in journal
	// will cause an error instead of being dropped.如果出现在日志中的损坏或无效的块或块。	将导致错误而不是被丢弃。这将防止打开损坏日志的数据库。
	...
	StrictBlockChecksum //严格块校验和

	// If present then a corrupted 'sorted table' will fails compaction.
	// The database will enter read-only mode.如果存在，那么损坏的“排序表”将失败压缩。 数据库将进入只读模式。
	StrictCompaction //严格的压实

	// If present then a corrupted 'sorted table' will halts 'read operation'.如果存在，则损坏的“排序表”将停止“读取操作”
	StrictReader //严格的读取

	// If present then leveldb.Recover will drop corrupted 'sorted table'.如果存在，则Level db.Recover将删除损坏的“排序表”。
	StrictRecovery //严格的恢复

	// This only applicable for ReadOptions, if present then this ReadOptions
	// 'strict level' will override global ones.这只适用于读取选项，如果存在，则此读取选项。“严格级别”将凌驾于全球水平之上。
	StrictOverride //严格的覆盖
	...
	// will override default strict flags as whole (i.e. not OR'ed).DefaultStrict是默认的严格标志。指定任何严格标志 将覆盖默认严格标志作为整体(即不OR‘ed)。
	DefaultStrict = StrictJournalChecksum | StrictBlockChecksum | StrictCompaction | StrictReader

	// NoStrict disables all strict flags. Override default strict flags.NoStrict禁用所有严格的标志。覆盖默认严格标志。
	NoStrict = ^StrictAll
)
...
type ReadOptions struct {
	...
	//DontFillCache定义块是否读取此“读取操作” 应该被缓存。如果为false，则将缓存块。这确实是 不影响已经缓存的块。 默认值为false。
	// The default value is false.
	DontFillCache bool

	// Strict will be OR'ed with global DB 'strict level' unless StrictOverride
	// is present. Currently only StrictReader that has effect here.除非严格覆盖，否则将使用全局DB“严格级别”进行或“编辑”。 是存在的。目前只有严格的读者，有效力在这里。
	Strict Strict
}
