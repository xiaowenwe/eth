...
// os.ErrExist error.打开或创建给定存储的数据库。如果不存在，则将创建数据库，除非出错为true。而且，如果错误确实存在，并且数据库存在打开将返回操作系统错误。
//
...
//如果损坏，打开将返回错误，错误类型错误在DB中检测到。使用错误。已损坏以测试错误是否因为腐败。损坏的DB可以恢复功能恢复。

// The returned DB instance is safe for concurrent use.返回的数据库实例对于并发使用是安全的。在使用后，通过调用Close方法，必须关闭DB。
...
// of the returned slice获取给定密钥的值。如果DB不包含密钥，则返回ErrNotFound。返回的片是它自己的副本，修改返回片的内容是安全的。在GET返回后修改参数的内容是安全的。.
...
// Has returns true if the DB does contains the given key.如果DB包含给定的键，则返回true。
//
// It is safe to modify the contents of the argument after Has returns.有返回后修改参数的内容是安全的。
func (db *DB) Has(key []byte, ro *opt.ReadOptions) (ret bool, err error) {
	...
	se := db.acquireSnapshot()   //获取快照
	defer db.releaseSnapshot(se) //释放快照
	return db.has(nil, nil, key, se.seq, ro)
}

//NewIterator returns an iterator for the latest snapshot of the
//underlying DB.NewIterator返回基础DB的最新快照的迭代器。
//The returned iterator is not safe for concurrent use, but it is safe to use
//multiple iterators concurrently, with each in a dedicated goroutine.返回的迭代器对于并发使用是不安全的，但是并发使用多个迭代器是安全的，每个迭代器都位于一个专用的goroutine中。
//It is also safe to use an iterator concurrently with modifying its
//underlying DB. The resultant key/value pairs are guaranteed to be
//consistent.同时使用迭代器并修改其底层DB也是安全的。得到的键/值对保证是一致的。
//
//Slice allows slicing the iterator to only contains keys in the given
//range. A nil Range.Start is treated as a key before all keys in the
//DB. And a nil Range.Limit is treated as a key after all keys in
//the DB.片允许将迭代器切片只包含给定范围内的键。启动在DB中的所有键之前被视为键。和一个零值，限制被视为在DB中所有键之后的键。
//迭代器必须在使用后通过调用Relation方法释放。
//The iterator must be released after use, by calling Release method.
//还可以阅读平面db/iterator包的Iterator文档。
...
func (db *DB) NewIterator(slice *util.Range, ro *opt.ReadOptions) iterator.Iterator {
	...
	// can be released after iterator created.Iterator持有‘version’lock，‘version’是不可变的，所以在迭代器创建后可以释放快照。
