...
func (db *DB) memCompaction() {
	mdb := db.getFrozenMem() // rotateMem的时候把当前使用的memory指向到frozonMem，这里读出来写入level 0文件
	if mdb == nil {
		return
	}
	defer mdb.decref()
	...
	// Commit.// 就是最终存储tables，写入到version记录。。。后续深入看下
	stats.startTimer()
...
//compaction是把数据一级一级的往下写，leveldb实现了minor compaction和major compaction minor compaction，leveldb里面的mCompaction goroutine做的事情，就是把memory中的数据写入到level 0文件中
