...
// Don't modify; this value are saved to disk.值类型编码为内部键的最后一个组件。 不要修改；此值保存到磁盘。
const (
...
// we need to use the highest-numbered ValueType, not the lowest).keyTypeSeek定义了构造
//用于查找特定序列号的内部密钥(因为我们
//按递减顺序排序序列号，值类型为
//嵌入在内部密钥的序列号中的低8位，我们需要使用编号最高的值类型，而不是最低值类型
const keyTypeSeek = keyTypeVal

const (
	// Maximum value possible for sequence number; the 8-bits are
	// used by value type, so its can packed together in single
	// 64-bit integer.序列号的最大值；8位是	由值类型使用，因此它可以单独包装在一起。64位整数
	keyMaxSeq = (uint64(1) << 56) - 1
	// Maximum value possible for packed sequence number and type.//包装序列号和类型的最大值
