...
type set struct {
	m map[interface{}]struct{} // struct{} doesn't take up space/struct {}不占用空间
}
...
// NewNonTS创建并初始化一个新的非线程安全集。
//它接受可变数量的参数来填充初始集。
//如果没有传递任何内容，则创建一个零大小的SetNonTS。
...
//添加包括指定的项目（一个或多个）到集合。 潜在的
//集合已被修改。 如果没有通过，它会默默地返回。
