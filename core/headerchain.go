...
//HeaderChain实现了core.BlockChain和Light.LightChain共享的基本块头链逻辑。
// 它本身是不可用的，只能作为/一种结构的一部分使用。
// 它也不是线程安全的，封装的链式结构应该做必要的互斥锁/解锁。
...
type HeaderChain struct {
	...
	currentHeader     atomic.Value // Current head of the header chain (may be above the block chain!)标题链的当前头部（可能在块链上方！）
	currentHeaderHash common.Hash  // Hash of the current head of the header chain (prevent recomputing all the time)标题链当前头部的哈希值（防止重新计算）
...
//getValidator应该将父服务器的验证器/prointerinterpoint返回到父服务器的中断信号量/wg指向父服务器的关机等待组。
...
///GetBlockNumber从缓存或数据库检索属于给定散列/的块号
...
///GetHeaderByNumber通过编号从数据库检索块头，如果找到/缓存它(与其哈希关联)。
...
//CurrentHeader检索规范链的当前头标题。/头从HeaderChain的内部缓存中检索。
...
//DeleteCallback是SetHead在/每个头被删除之前调用的回调函数。
// DeleteCallback is a callback function that is called by SetHead before
// each header is deleted.
type DeleteCallback func(common.Hash, uint64)
//SetHead将本地链恢复到一个新的头。新标题/上方的所有内容将被删除，新的一套将被删除。
