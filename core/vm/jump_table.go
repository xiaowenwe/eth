...
//jumptable. 是一个 [256]operation 的数据结构. 每个下标对应了一种指令,
// 使用operation来存储了指令对应的处理逻辑, gas消耗, 堆栈验证方法, memory使用的大小等功能.
type (
...
//数据结构operation存储了一条指令的所需要的函数.operation在操作过程中，会需要几个数据结构： Stack，实现了标准容器 -栈的行为；
// Memory，一个字节数组，可表示线性排列的任意数据;还有一个intPool，提供对big.Int数据的存储和读取。
type operation struct {
	// op is the operation function执行函数
...
NewByzantiumInstructionSet 拜占庭版本首先调用NewHomesteadInstructionSet创造了前一个版本的指令,
然后增加自己特有的指令.STATICCALL ,RETURNDATASIZE ,RETURNDATACOPY ,REVERT*/
var (
...
// NewConstantinopleInstructionSet返回边界，宅基地拜占庭和君士坦丁堡的指令。
...
// NewHomesteadInstructionSet返回边界和宅基地
//在宅基地阶段可以执行的指令。
