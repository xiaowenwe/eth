...
//Etalum协议中的/StateDB用于存储任何/在MerkelTrie中的任何内容。StateDB负责缓存和存储/嵌套状态。它是检索：/*Contracts/*帐户的通用查询接口
...
type StateDB struct {
	...
	/*StateDB 在内部用一个巨大的map结构来管理这些stateObject对象。所有账户信息-包括Ether余额，指令数组Code, 该账户发起合约次数nonce等-它们发生的所有变化，会首先缓存到StateDB里的某个stateObject里，然后在合适的时候，被StateDB一起提交到底层数据库。
	注意，一个Contract所对应的stateObject的地址，是Contract的self地址，也就是转帐的转入方地址*/
	// 下面的Map用来存储当前活动的对象，这些对象在状态转换的时候会被修改。
	// stateObjects 用来缓存对象
	...
	// DB error.
	//状态对象由共识核心和VM使用，它们无法处理数据库级错误。 在数据库读取期间发生的任何错误都会在此处进行记忆，并最终由StateDB.Commit返回。
...
//重置清除状态数据库中的所有临时状态对象，但保留
//基础状态试图避免为下一个操作重新加载数据。
...
//在交易执行过程中被VM执行。添加日志。同时把日志和交易关联起来，添加部分交易的信息。
...
// Exist报告状态中是否存在给定的帐户地址。
//值得注意的是，对于自杀帐户，这也会返回true。
...
func (self *StateDB) getStateObject(addr common.Address) (stateObject *stateObject) {
	...
	// Insert into the live set.//插入实时集。
...
// createObject创建一个新的状态对象。 如果有现有帐户
//给定的地址，它被覆盖并作为第二个返回值返回。
...
// RevertToSnapshot恢复自给定修订以来所做的所有状态更改。
// RevertToSnapshot reverts all state changes made since the given revision.
func (self *StateDB) RevertToSnapshot(revid int) {
	// Find the snapshot in the stack of valid snapshots.//在有效快照堆栈中查找快照。
	...
	// Replay the journal to undo changes.//重播日志以撤消更改。
...
// 这个方法会在交易执行的过程中被调用。会被存入 transaction receipt
//Finalise方法会调用update方法把存放在cache层的修改写入到trie数据库里面。
// 但是这个时候还没有写入底层的数据库。
...
// IntermediateRoot计算状态trie的当前根散列。
//在交易之间调用它以获取进入交易接收的根散列
