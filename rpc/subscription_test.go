...
func (s *NotificationTestService) SomeSubscription(ctx context.Context, n, val int) (*Subscription, error) {
	...
	//通过显式创建订阅，我们确保在第一次订阅之前将订阅ID发送回客户端。调用Notify。 否则，事件可能会在eth_subscribe方法的响应之前发送。
	...
	go func() {
		//如果我们开始立即发送事件某些事件，则测试需要n个事件	可能会被删除，因为订阅ID可能不会发送到客户端。
...
//在发送任何内容之前，挂起s.sunblockHangSubscription上的HangSubscription块。
...
func TestNotifications(t *testing.T) {
	...
	go server.ServeSingleRequest(NewJSONCodec(serverConn), OptionMethodInvocation|OptionSubscriptions)
	...
	var ok bool
	var a string
	if a, ok = response.Result.(string); !ok {
	...
	clientConn.Close() // causes notification unsubscribe callback to be called导致通知取消订阅回调被调用
...
// TestSubscriptionMultipleNamespaces确保订阅可以存在于多个不同的命名空间。
...
func TestSubscriptionMultipleNamespaces(t *testing.T) {
	...
	for {
		...
		case notification := <-notifications:
			...
			fmt.Println(count[notification.Params.Subscription])
		case <-timeout:
			for _, namespace := range namespaces {
