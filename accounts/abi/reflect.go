...
// reflectIntKind使用给定的大小和unsignedness返回反射。
...
// mustArrayToBytesSlice创建一个与value完全相同的新字节切片，并将value中的字节复制到新切片。
...
// set在赋值时稍微宽松一点，并且不强制像“反射”那样严格的规则集。
...
// requireAssignable确保`dest`是一个指针，它不是一个接口。
...
// requireUnpackKind验证将`args`解压缩为`kind`的前提条件
...
// requireUniqueStructFieldNames确保字段名称不会发生冲突
