...
type tmplData struct {
	Package   string                   // Name of the package to place the generated file in放置生成的文件的包的名称
	Contracts map[string]*tmplContract // List of contracts to generate into this file要生成到此文件中的合同列表
}

// tmplContract contains the data needed to generate an individual contract binding.// tmplContract包含生成单个合同绑定所需的数据。
type tmplContract struct {
	Type        string                 // Type name of the main contract binding键入主合同绑定的名称
	InputABI    string                 // JSON ABI used as the input to generate the binding fromJSON ABI用作生成绑定的输入
	InputBin    string                 // Optional EVM bytecode used to denetare deploy code from用于从中删除部署代码的可选EVM字节码
	Constructor abi.Method             // Contract constructor for deploy parametrization用于部署参数化的合同构造函数
	Calls       map[string]*tmplMethod // Contract calls that only read state data合同调用只读取状态数据
	Transacts   map[string]*tmplMethod // Contract calls that write state data写入状态数据的合同调用
	Events      map[string]*tmplEvent  // Contract events accessors合同事件访问者
}

// tmplMethod是abi.Method的包装器，它包含一些预处理和缓存的数据字段。
// tmplMethod is a wrapper around an abi.Method that contains a few preprocessed
// and cached data fields.
type tmplMethod struct {
	Original   abi.Method // Original method as parsed by the abi package由abi包解析的原始方法
	Normalized abi.Method // Normalized version of the parsed method (capitalized names, non-anonymous args/returns)已解析方法的规范化版本（大写名称，非匿名args /返回）
	Structured bool       // Whether the returns should be accumulated into a struct是否应将回报累积到结构中
}

// tmplEvent是a的包装器
// tmplEvent is a wrapper around an a
type tmplEvent struct {
	Original   abi.Event // Original event as parsed by the abi package由abi包解析的原始事件
	Normalized abi.Event // Normalized version of the parsed fields已解析字段的规范化版本
}

// tmplSource是语言到模板的映射，包含程序包可以生成的所有支持的编程语言。
...
// tmplSourceJava是用于生成基于合同绑定的Java源模板。
