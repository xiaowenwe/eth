...
type unpackTest struct {
	def  string      // ABI definition JSON// ABI定义JSON
	enc  string      // evm return data/evm返回数据
	want interface{} // the expected output/预期的产出
	err  string      // empty or error if expected/如果预期，则为空或错误
}
...
func TestMultiReturnWithDeeplyNestedArray(t *testing.T) {
	//类似于TestMultiReturnWithArray，但考虑到一个特殊情况：
	//嵌套静态数组的值也会计入大小，并且应该使用正确的偏移量读取此类嵌套数组参数后面的任何元素，
	//这样它就不会读取前一个数组参数中的内容。
	...
	buff := new(bytes.Buffer)
	//构造测试数组，每个3个char元素与61个'0'字符连接，
	//来自数组中的（（3 + 61）* 0.5）= 32字节元素。
	...
	buff.Write(common.Hex2Bytes(strings.Join([]string{
		"", //empty, to apply the 61-char separator to the first element as well.为空，将61字符分隔符应用于第一个元素。
