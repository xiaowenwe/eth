...
func TestWatchNewFile(t *testing.T) {
	...
	for d := 200 * time.Millisecond; d < 5*time.Second; d *= 2 {
		...
		if reflect.DeepEqual(list, wantAccounts) {
			// ks should have also received change notifications// ks也应该收到更改通知
			select {
			case <-ks.changes:
			default:
...
func TestCacheAddDeleteOrder(t *testing.T) {
	...
	cache.watcher.running = true // prevent unexpected reloads防止意外重新加载
...
// TestUpdatedKeyfileContents测试，观察者注意到更新密钥库文件的内容，并相应地更新帐户缓存
...
func TestUpdatedKeyfileContents(t *testing.T) {
	...
	//需要使`file`的modTime与forceCopyFile之后的当前值不同
	...
	//需要使`file`的modTime与ioutil.WriteFile之后的当前值不同
...
// forceCopyFile与cp.CopyFile类似，但如果目标存在则不会抱怨。
