...
// NoOdr是不需要ODR服务时传递给支持ODR的函数的默认上下文。
...
// OdrRequest is an interface for retrieval requests OdrRequest是检索请求的接口
type OdrRequest interface {
	StoreResult(db ethdb.Database)
}

// TrieID identifies a state or account storage trie TrieID标识状态或帐户存储trie
type TrieID struct {
...
// StateTrieID为属于某个块头的状态trie返回TrieID。
...
// StorageTrieID为给定状态trie的给定帐户返回合同存储trie的TrieID。 它还需要trie的根哈希来检查Merkle样张
