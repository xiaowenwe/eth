...
// NewTxPool creates a new light transaction pool NewTxPool创建一个新的轻型事务池
...
// currentState returns the light state of the current head header currentState返回当前头标题的light状态
func (pool *TxPool) currentState(ctx context.Context) *state.StateDB {
	return NewState(ctx, pool.chain.CurrentHeader(), pool.odr)
}

// GetNonce返回给定地址的“待定”随机数。 它总是查询属于最新头的随机数，以便检测使用相同密钥的另一个客户端是否发送了一个事务。
