...
// NewLightChain使用数据库中提供的信息返回完全初始化的轻链。 它初始化默认的以太坊标头验证器。
...
// loadLastState从数据库加载最后一个已知的链状态。 此方法假定链管理器互斥锁被保留。
...
// State returns a new mutable state based on the current HEAD block.State根据当前的HEAD块返回一个新的可变状态。
...
// GetBlock通过哈希和数字从数据库或ODR服务中检索块， 如果找到则缓存它。
// GetBlock retrieves a block from the database or ODR service by hash and number,
// caching it if found.
func (self *LightChain) GetBlock(ctx context.Context, hash common.Hash, number uint64) (*types.Block, error) {
	// Short circuit if the block's already in the cache, retrieve otherwise如果块已经在缓存中则短路，否则检索
