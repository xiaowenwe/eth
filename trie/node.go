...
type (
	fullNode struct {
		Children [17]node // Actual trie node data to encode/decode (needs custom encoder) //对应了黄皮书里面的分支节点,要编码/解码的实际节点数据（需要自定义编码器）
		flags    nodeFlag
	}
...
type nodeFlag struct {
	...
	hash  hashNode // cached hash of the node (may be nil)//节点的缓存哈希值（可能为零）
	gen   uint16   // cache generation counter缓存生成计数器只要对应的node发生一次变化，计数就加一
	dirty bool     // whether the node has changes that must be written to the database节点是否具有必须写入数据库的更改
}

// canUnload告诉您是否可以卸载节点。
