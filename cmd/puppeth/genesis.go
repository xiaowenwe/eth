...
// cppEthereumGenesisSpec表示使用的起源规范格式
// C ++以太坊实现
...
// cppEthereumGenesisSpecBuiltin是预编译的合同定义。
...
// newCppEthereumGenesisSpec将go-ethereum genesis块转换为Parity特定链规范格式。
...
func newCppEthereumGenesisSpec(network string, genesis *core.Genesis) (*cppEthereumGenesisSpec, error) {
	// Only ethash is currently supported between go-ethereum and cpp-ethereum// go-ethereum和cpp-ethereum目前仅支持ethash
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Parity's format//以Parity格式重建链规范
...
// pyEthereumGenesisSpec表示使用的起源规范格式
// Python以太坊实现
...
// newPyEthereumGenesisSpec将go-ethereum起源块转换为奇偶校验特定链规范格式
// newPyEthereumGenesisSpec converts a go-ethereum genesis block into a Parity specific
// chain specification format.
func newPyEthereumGenesisSpec(network string, genesis *core.Genesis) (*pyEthereumGenesisSpec, error) {
	// Only ethash is currently supported between go-ethereum and pyethereum
	// go-ethereum和pyethereum目前仅支持ethash
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
