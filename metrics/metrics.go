...
// CollectProcessMetrics会定期收集有关正在运行的进程的各种度量标准。
// CollectProcessMetrics periodically collects various metrics about the running
// process.
func CollectProcessMetrics(refresh time.Duration) {
	// Short circuit if the metrics system is disabled如果禁用度量系统，则短路
	if !Enabled {
		return
	}
	// Create the various data collectors创建各种数据收集器
	...
	// Define the various metrics to collect定义要收集的各种指标
