...
//调用以params作为输入值调用（常量）contract方法，并将输出设置为result。 结果类型可以是简单返回的单个字段，匿名返回的接口切片和命名返回的结构。
...
// ChequebookOverdraftIterator从FilterOverdraft返回，用于迭代Checkbook合同引发的Overdraft事件的原始日志和解压缩数据。
