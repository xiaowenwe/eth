...
// PublicResolverBin is the compiled bytecode used for deploying new contracts.PublicResolverBin是用于部署新合同的已编译字节码。
...
// DeployPublicResolver部署一个新的以太坊契约，将PublicResolver的实例绑定到它。
...
//// NewPublicResolverFilterer创建PublicResolver的新日志过滤器实例，绑定到特定的已部署合同。
...
// PublicResolverABIChanged表示PublicResolver合同引发的ABIChanged事件。
...
// PublicResolverAddrChangedIterator从FilterAddrChanged返回，用于迭代PublicResolver合同引发的AddrChanged事件的原始日志和解压缩数据
...
//
// Solidity：事件AddrChanged（节点索引bytes32，地址）
...
// Next将迭代器推进到后续事件，返回是否还有其他事件。 在检索或解析错误的情况下，返回false并且可以查询Error（）以确定故障。
// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
