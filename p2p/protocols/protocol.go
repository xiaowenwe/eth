...
软件包协议是p2p的扩展。 它提供了一种用户友好的简单方式来定义
devp2p子协议通过抽象掉协议标准共享的代码。
...
//运行开始处理在p2p.Protocol中调用的传入消息的永久循环＃运行函数处理程序参数是为从远程对等接收的每个消息调用的函数，返回的错误导致循环退出导致断开连接
...
// handleIncoming（code）被称为主永久循环的每个周期，调度传入的消息，如果这返回一个错误，循环返回并且对等体与错误断开连接
// *检查邮件大小，
// *检查超出范围的消息代码，
// *用反射来处理解码，
// *将处理程序作为回调调用
...
func (p *Peer) handleIncoming(handle func(msg interface{}) error) error {
	...
	//调用注册的处理程序回调 一个已注册的回调函数将解码后的消息作为参数作为接口，
	// 处理程序应该将其转换为适当的类型，因为处理程序根据正确的类型进行选择，
	// 因此完全不会检查处理程序中的转换
