...
func TestUPNP_DDWRT(t *testing.T) {
	...
	dev := &fakeIGD{
		...
		httpResps: map[string]string{
			...
			// namespaced.
			//对GetNATRSIPStatus调用的响应。 这个特定的实现有一个错误：u：GetNATRSIPStatusResponse中的元素没有正确的命名空间。
...
// fakeIGD将自己呈现为可发现的UPnP设备，该设备向HTTPU和HTTP请求发送预设响应。
...
type fakeIGD struct {
	...
	//这应该是一个完整的HTTP响应（包括头文件）。
	//它作为对任何sspd数据包的响应发送。 任何出现的“{{listenAddr}}”被替换为HTTP服务器的实际TCP侦听地址。
	...
	ssdpResp string
	//这个应该包含执行所有请求的XML有效载荷。 密钥包含方法和路径，例如 “GET / foo / bar”。 与ssdpResp一样，“{{listenAddr}}”被替换为TCP监听地址。
