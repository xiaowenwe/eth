...
// ListenUDP returns a new table that listens for UDP packets on laddr.ListenUDP返回一个新表，用于侦听laddr上的UDP数据包。
...
// readLoop在自己的goroutine中运行。 它将入口UDP数据包注入网络循环。
// readLoop runs in its own goroutine. it injects ingress UDP packets
// into the network loop.
func (t *udp) readLoop() {
	defer t.conn.Close()
	//发现数据包定义为不大于1280字节。	大于此大小的数据包将在末尾被剪切并视为无效，因为它们的哈希值不匹配。
