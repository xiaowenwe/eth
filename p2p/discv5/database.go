...
var (
	nodeDBNilNodeID      = NodeID{}       // Special node ID to use as a nil element.用作零元素的特殊节点ID。
	nodeDBNodeExpiration = 24 * time.Hour // Time after which an unseen node should be dropped.一段时间之后，一个看不见的节点应该被删除。
	nodeDBCleanupCycle   = time.Hour      // Time period for running the expiration task.运行过期任务的时间段。
)
...
type nodeDB struct {
	...
	self   NodeID        // Own node id to prevent adding it into the database自己的节点id，以防止将其添加到数据库中。
	runner sync.Once     // Ensures we can start at most one expirer/确保我们最多可以开始
	quit   chan struct{} // Channel to signal the expiring thread to stop/通道向即将到期的线程发出信号以停止}
}

// Schema layout for the node database//节点数据库的Schema布局
var (
	nodeDBVersionKey = []byte("version") // Version of the database to flush if changes如果更改，则刷新数据库的版本。
