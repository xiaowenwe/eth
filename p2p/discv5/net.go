...
type Network struct {
	db          *nodeDB // database of known nodes已知节点数据库
	conn        transport
	netrestrict *netutil.Netlist

	closed           chan struct{}          // closed when loop is done当循环完成时关闭
	closeReq         chan struct{}          // 'request to close'请求关闭“
	refreshReq       chan []*Node           // lookups ask for refresh on this channel查找请求在此通道上刷新
	refreshResp      chan (<-chan struct{}) // ...and get the channel to block on from this one.把这个频道封锁起来
	read             chan ingressPacket     // ingress packets arrive here入口包到达这里
	timeout          chan timeoutEvent
	queryReq         chan *findnodeQuery // lookups submit findnode queries on this channel查找在此通道上提交查找节点查询。
	tableOpReq       chan func()
	tableOpResp      chan struct{}
	topicRegisterReq chan topicRegisterReq
	topicSearchReq   chan topicSearchReq

	// State of the main loop.主回路的状态。
	tab           *Table
	topictab      *topicTable
	ticketStore   *ticketStore
	nursery       []*Node
	nodes         map[NodeID]*Node // tracks active nodes with state != known跟踪状态为！=已知的活动节点
	timeoutTimers map[timeoutEvent]*time.Timer
	//重新验证队列。/这些队列上的节点最终将被平移。
	// Revalidation queues.
...
//传输是由UDP传输实现的。它是一个接口，因此我们可以在不打开大量UDP
// 套接字和不生成私钥的情况下进行测试。
...
//nodeNetGuts嵌入Node并包含字段。
// nodeNetGuts is embedded in Node and contains fields.
type nodeNetGuts struct {
	//这是sha3(ID)的缓存副本，用于节点/距离计算。这是Node的一部分，以便能够编写需要一定距离的节点的测试。
	// /在这些测试中，SAA的内容实际上不会与ID对应。
	...
	sha common.Hash
	//状态机字段。访问这些字段
	//限制在Network.loop goroutine身上。
	// State machine fields. Access to these fields
	// is restricted to the Network.loop goroutine.
	state             *nodeState
	pingEcho          []byte           // hash of last ping sent by us我们发送的最后一个ping的散列
	pingTopics        []Topic          // topic set sent by us in last ping最后一页中我们发送的主题集
	deferredQueries   []*findnodeQuery // queries that can't be sent yet还不能发送的查询
	pendingNeighbours *findnodeQuery   // current query, waiting for reply当前查询，等待答复
	queryTimeouts     int
}
