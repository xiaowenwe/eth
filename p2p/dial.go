...
// NodeDialer用于连接网络中的节点，通常通过使用底层net.Dialer，但也使用net.Pipe进行测试
...
// TCPDialer通过使用net.Dialer实现NodeDialer接口来创建到网络中节点的TCP连接
...
// dialstate计划拨号和发现查找。
//它有机会在Server.run的主循环的每次迭代中计算新任务。
...
//为每个拨打的节点生成dialTask。 任务运行时无法访问其字段。
...
// dailTask主要负责建立链接。 如果t.dest是没有ip地址的。
// 那么尝试通过resolve查询ip地址。 然后调用dial方法创建链接。
// 对于静态的节点。如果第一次失败，那么会尝试再次resolve静态节点。
// 然后再尝试dial（因为静态节点的ip是配置的。 如果静态节点的ip地址变动。
// 那么我们尝试resolve静态节点的新地址，然后调用链接。）
