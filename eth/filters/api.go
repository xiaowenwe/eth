...
//eth/filter 包 包含了给用户提供过滤的功能，用户可以通过调用对交易或者区块进行过滤，
// 然后持续的获取结果，如果5分钟没有操作，这个过滤器会被删除。
var (
...
type filter struct {
	...
	deadline *time.Timer    // filter is inactiv when deadline triggers// filter is inactiv when deadline triggers 当计时器响起的时候，会触发定时器。
	hashes   []common.Hash  //过滤出来的hash结果
	crit     FilterCriteria //过滤条件
	logs     []*types.Log   //过滤出来的Log信息
	s        *Subscription  // associated subscription in event system// associated subscription in event system 事件系统中的订阅器。
}
...
// NewPublicFilterAPI返回一个新的PublicFilterAPI实例。
...
//NewPendingTransactionFilter,用来创建一个PendingTransactionFilter。
// 这种方式是用来给那种无法创建长连接的通道使用的(比如HTTP),
...
//对于可以建立长连接的通道，可以直接使用rpc的发送订阅模式， 这样客户端就可以直接接收到过滤信息，不用调用轮询的方式了。
// 可以看到这种模式下面并没有添加到filters这个容器，也没有超时管理了。也就是说支持两种模式。
...
//日志创建一个订阅，该订阅将触发与给定过滤条件匹配的所有新日志。
...
//日志过滤功能，根据FilterCriteria指定的参数，来对日志进行过滤，开始区块，结束区块，地址和Topics，
// 这里面引入了一个新的对象filter
...
//用来返回从上次调用到现在的所有的指定id的所有过滤信息。这个可以用来轮询。
