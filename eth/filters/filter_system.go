...
// Type确定过滤器的类型，用于在添加过滤器时将过滤器放入正确的桶中。
...
// EventSystem创建订阅，处理事件并将它们广播到与订阅条件匹配的订阅。
...
// NewEventSystem创建一个新的管理器，用于侦听给定多路复用器上的事件，
//解析并过滤它们。 它使用all map来检索过滤器更改。 工作循环拥有自己的索引，用于将事件转发到过滤器。
//返回的管理器有一个循环，需要使用Stop函数停止或停止给定的多路复用器。
...
func (es *EventSystem) eventLoop() {
	var (
		...
		// Subscribe TxPreEvent form txpool//订阅TxPreEvent表格txpool
