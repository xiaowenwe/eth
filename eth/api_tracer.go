...
///traceChain根据所提供的配置一个新的跟踪器，并/执行其中包含的所有事务。返回值将是一个项/每个事务，取决于请求跟踪程序。
...
///ComputeStateDB检索与某个块关联的状态数据库。/如果给定块没有本地可用的状态，则会重新执行许多块以生成所需的状态。
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	for block.NumberU64() < origin {
		...
		// Retrieve the next block to regenerate and process it/检索下一个块以重新生成并处理它
