...
// PublicDownloaderAPI提供了一个API，提供有关当前同步状态的信息。
//它仅提供对数据进行操作的方法，这些方法可供没有安全风险的任何人使用。
...
// NewPublicDownloaderAPI创建一个新的PublicDownloaderAPI。 API有一个内部事件循环，通过全局事件多路复用器监听来自下载程序的事件。
// 如果它收到这些事件之一，它会将其广播到通过installSyncSubscription通道安装的所有同步订阅。
...
// eventLoop运行循环，直到事件mux关闭。 它将安装和卸载新的同步订阅，并向已安装的同步订阅广播同步状态更新。
