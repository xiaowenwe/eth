...
// meteredMsgReadWriter是一个p2p.MsgReadWriter的包装，能够基于数据流内容累积上面定义的度量。
// meteredMsgReadWriter is a wrapper around a p2p.MsgReadWriter, capable of
// accumulating the above defined metrics based on the data stream contents.
type meteredMsgReadWriter struct {
	p2p.MsgReadWriter     // Wrapped message stream to meter包裹的消息流到仪表
	version           int // Protocol version to select correct meters协议版本选择正确的仪表
}

// newMeteredMsgWriter用测量支持包装一个p2p MsgReadWriter。 如果度量系统被禁用，该函数返回原始对象。
...
// Init设置流使用的协议版本，以便在协议版本之间重叠消息ID时知道哪些仪表要增加。
