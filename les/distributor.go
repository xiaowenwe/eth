...
// distReq是分发者使用的请求抽象。 它基于
//三个回调函数：
//  -  getCost返回向给定对等方发送请求的成本的上限估计值
//  -  canSend告诉服务器对等体是否适合提供请求
//  - 请求准备将请求发送给给定的对等方并返回一个函数
//实际发送。 应该保留请求顺序，但回调本身不应该
//阻塞直到它被发送，因为其他对等方可能仍然能够接收请求
//其中一个是阻塞 相反，返回的函数放在对等体的发送队列中。
...
// newRequestDistributor creates a new request distributor newRequestDistributor创建一个新的请求分发者
	...
	for {
		...
		loop:
			for {
				...
				if req != nil && wait == 0 {
					chn := req.sentChn // save sentChn because remove sets it to nil  save sendChn，因为remove将其设置为nil
					d.remove(req)
					...
					if wait == 0 {
						...
						// queued request will wake up the loop//无需发送请求，无需等待; 下一个排队的请求将唤醒循环
						break loop
					}
					d.loopNextSent = true // a "next" signal has been sent, do not send another one until this one has been received  已发送“下一个”信号，在收到此信号之前不发送另一个信号
					if wait > distMaxWait {
						// waiting times may be reduced by incoming request replies, if it is too long, recalculate it periodically  来电请求回复可能会减少等待时间，如果时间太长，请定期重新计算
						wait = distMaxWait
					}
					go func() {
						time.Sleep(wait)
...
// nextRequest返回来自任何对等方的下一个可能请求，以及相关的对等方和必要的等待时间
