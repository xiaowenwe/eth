...
// bridge是一个JavaScript实用程序方法的集合，用于新娘.js运行时
...
// newBridge在RPC客户端周围创建一个新的JavaScript包装器。
...
// NewAccount是personal.newAccount RPC方法的包装器，它使用非回显密码提示来获取密码并执行原始密码
// RPC方法（保存在jeth.newAccount中）与它实际执行RPC调用。
...
// OpenWallet是personal.openWallet的包装器，可以解释和响应某些错误消息，例如Trezor PIN矩阵请求。
